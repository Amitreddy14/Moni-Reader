...
function registerEditKeybinds() {
    ...
    // edit text boxes.
    keybinds.registerKeybind(ReaderModality.EDIT, "Escape", "Exit edit mode", () => {
        popMode();
...
function registerNormalKeybinds() {
    ...
    keybinds.registerKeybind(ReaderModality.NORMAL, "ArrowDown", "Read the next element", normalRead);
    // arrowup to go back to previous element
    keybinds.registerKeybind(ReaderModality.NORMAL, "ArrowUp", "Go back to the previous element", () => {
        ...
        for (; prevIdx < all.length; prevIdx++) {
            ...
            if (all[prevIdx].classList.contains("__current_sr_elt"))
                break;
        }
        if (prevIdx === all.length || prevIdx === 0) {
            ...
            do {
                i++;
                if (all[prevIdx - i].id.startsWith("__sr"))
                    continue;
                prevString = elementToString(all[prevIdx - i]);
            } while (!prevString);
            // focus next element and speak out the element string
            all[prevIdx - i].classList.add("__current_sr_elt");
            all[prevIdx - i].focus();
            let stringified = elementToString(all[prevIdx - i]);
    ...
    keybinds.registerKeybind(ReaderModality.NORMAL, "ArrowLeft", "Slow down", () => synth.decreaseVoiceRate());
    // arrowright to speed up
    keybinds.registerKeybind(ReaderModality.NORMAL, "ArrowRight", "Speed up", () => synth.increaseVoiceRate());
    // t to enter table mode. We may refuse to enter if the current highlighted
    // element is not a <table>.
    keybinds.registerKeybind(ReaderModality.NORMAL, "t", "Enter table mode", () => {
        let all = document.getElementsByTagName("*");
        let prevIdx = 0;
        for (; prevIdx < all.length; prevIdx++) {
            if (all[prevIdx].classList.contains("__current_sr_elt"))
    ...
    keybinds.registerKeybind(ReaderModality.NORMAL, " ", `Click the hovered element`, () => {
...
function registerTableKeybinds() {
    ...
    keybinds.registerKeybind(ReaderModality.TABLE, "ArrowRight", "Move right", () => {
        let all = document.getElementsByTagName("*");
        let prevIdx = 0;
        for (; prevIdx < all.length; prevIdx++) {
            if (all[prevIdx].classList.contains("__current_sr_elt"))
        ...
        if (prevIdx === all.length)
            return;
        let prevNode = all[prevIdx];
        ...
        if (prevNodeIdx < siblings.length - 1) {
            ...
            siblings[prevNodeIdx + 1].focus();
            siblings[prevNodeIdx + 1]
    ...
    keybinds.registerKeybind(ReaderModality.TABLE, "ArrowLeft", "Move left", () => {
        let all = document.getElementsByTagName("*");
        let prevIdx = 0;
        for (; prevIdx < all.length; prevIdx++) {
            if (all[prevIdx].classList.contains("__current_sr_elt"))
    ...
    keybinds.registerKeybind(ReaderModality.TABLE, "ArrowDown", "Move down", () => {
        let all = document.getElementsByTagName("*");
        let prevIdx = 0;
        for (; prevIdx < all.length; prevIdx++) {
            if (all[prevIdx].classList.contains("__current_sr_elt"))
        ...
        while (tableElt.tagName !== "TABLE")
            tableElt = tableElt.parentElement;
        let rows = Array.from(tableElt.getElementsByTagName("tr"));
        let rowIdx = rows.indexOf(prevNode.parentElement);
        console.log(rowIdx);
        console.log(rows);
        if (rowIdx === rows.length - 1) {
            ...
            prevNode.classList.remove("__current_sr_elt");
            nextNode.focus();
            nextNode.classList.add("__current_sr_elt");
            readCell();
        }
    });
    ...
    keybinds.registerKeybind(ReaderModality.TABLE, "ArrowUp", "Move up", () => {
        let all = document.getElementsByTagName("*");
        let prevIdx = 0;
        for (; prevIdx < all.length; prevIdx++) {
            if (all[prevIdx].classList.contains("__current_sr_elt"))
...
function readCell() {
    ...
    if (prevIdx === all.length)
        ...
        let toSpeak = "";
        if (all[prevIdx].tagName === "TH")
            toSpeak += "Column header: ";
        else if (all[prevIdx].tagName === "TD")
            toSpeak += "Table cell: ";
        toSpeak += innerText(all[prevIdx]);
        synth.speak(toSpeak);
        return;
    }
}
...
function normalRead() {
    ...
    if (prevIdx === all.length - 1)
        return;
    else if (prevIdx === all.length)
        prevIdx = -1;
    else {
        all[prevIdx].blur();
        all[prevIdx].classList.remove("__current_sr_elt");
    ...
    let stringified = elementToString(all[prevIdx + 1]);
    if (stringified)
        synth.speak(stringified);
    else {
        normalRead();
...
function renderControls() {
    ...
    (_a = document.getElementById("__sr_help")) === null || _a === void 0 ? void 0 : _a.addEventListener("click", speakHelp);
    for (i = 0; i < cb.length; i++) {
        ...
        let f = cb[i];
        (_b = document.getElementById(`__sr_controls_${i}`)) === null || _b === void 0 ? void 0 : _b.addEventListener("click", f);
    }
}